              ...
              {lessons.map((lesson, i) => {
                ...
                return (
                  <React.Fragment key={lesson.id}>
                    ...
                    {/* صف العنوان الفرعي — يظهر فقط إذا كان description موجوداً */}
                    {lesson.description && (
                      <tr style={{ backgroundColor: i % 2 === 0 ? "#f0f7ff" : "#e8f4ff" }}>
                        ...
                        <td className="px-4 py-2 border border-gray-200 border-t-0 text-center">
                          <button
                            onClick={() => router.push(`/gratis/lesson?lessonId=${lesson.id}&category=${selectedCat}&lesson=${encodeURIComponent(lesson.description)}`)}
                        ...
                        <td className="px-4 py-2 border border-gray-200 border-t-0 text-center">
                          <button
                            onClick={() => router.push(`/gratis/exam?category=${selectedCat}&lessonId=${lesson.id}&lesson=${encodeURIComponent(lesson.description)}`)}
                            className="border-2 px-4 py-1 text-xs font-bold transition-colors w-full"
