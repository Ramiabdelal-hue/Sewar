...
function LessonsContent() {
  ...
  const getCategoryTitle = () => {
    if (currentCategory === "C") return lang === "ar" ? "‰Ÿ—Ì… —Œ’… «·ﬁÌ«œ… C" : lang === "nl" ? "THEORIE RIJBEWIJS C OEFENVRAGEN EN PROEFEXAMEN" : lang === "fr" ? "THEORIE PERMIS C QUESTIONS ET EXAMEN" : "THEORY LICENSE C PRACTICE QUESTIONS AND EXAM";
    if (currentCategory === "A") return lang === "ar" ? "‰Ÿ—Ì… —Œ’… «·ﬁÌ«œ… A" : lang === "nl" ? "THEORIE RIJBEWIJS A OEFENVRAGEN EN PROEFEXAMEN" : lang === "fr" ? "THEORIE PERMIS A QUESTIONS ET EXAMEN" : "THEORY LICENSE A PRACTICE QUESTIONS AND EXAM";
    return lang === "ar" ? "‰Ÿ—Ì… —Œ’… «·ﬁÌ«œ… B - √”∆·…  œ—Ì»Ì…" : lang === "nl" ? "THEORIE RIJBEWIJS B OEFENVRAGEN EN PROEFEXAMEN" : lang === "fr" ? "THEORIE PERMIS B QUESTIONS ET EXAMEN" : "THEORY LICENSE B PRACTICE QUESTIONS AND EXAM";
  };
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-bold text-gray-600">
            {lang === "ar" ? "Ã«—Ì «· Õ„Ì·..." : lang === "nl" ? "Laden..." : lang === "fr" ? "Chargement..." : "Loading..."}
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 p-6 text-center">
        <div className="bg-white p-10 rounded-2xl shadow-xl max-w-md border-t-8 border-red-500">
          ...
          <h2 className="text-2xl font-black text-gray-900 mb-3">
            {lang === "ar" ? "«·«‘ —«ﬂ €Ì— „ «Õ" : lang === "nl" ? "Abonnement niet beschikbaar" : lang === "fr" ? "Abonnement non disponible" : "Subscription not available"}
          </h2>
          <p className="text-gray-500 mb-6">
            {!userEmail
              ? (lang === "ar" ? "Ì—ÃÏ  ”ÃÌ· «·œŒÊ· „—… √Œ—Ï" : "Please login again")
              : (lang === "ar" ? "«‰ Â  ’·«ÕÌ… «‘ —«ﬂﬂ" : lang === "nl" ? "Uw abonnement is verlopen" : lang === "fr" ? "Votre abonnement a expirÈ" : "Your subscription has expired")}
          </p>
          <button
          ...
          >
            {!userEmail ? (lang === "ar" ? " ”ÃÌ· «·œŒÊ·" : "Login") : (lang === "ar" ? " ÃœÌœ «·«‘ —«ﬂ" : lang === "nl" ? "Vernieuwen" : lang === "fr" ? "Renouveler" : "Renew")}
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-white" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <div className="w-full px-4 py-6">
        ...
        <div className="mb-4">
          <input
            type="text"
            placeholder={lang === "ar" ? "«»ÕÀ ⁄‰ œ—”..." : lang === "nl" ? "Zoek een les..." : lang === "fr" ? "Rechercher..." : "Search lesson..."}
        ...
        {filteredLessons.length === 0 ? (
          <div className="border border-yellow-300 bg-yellow-50 p-6 text-center">
            <p className="font-bold text-gray-700">
              {lang === "ar" ? "·«  ÊÃœ œ—Ê” „ «Õ…" : lang === "nl" ? "Geen lessen beschikbaar" : lang === "fr" ? "Aucune leÁon disponible" : "No lessons available"}
            </p>
          </div>
        ) : (
              ...
              {filteredLessons.map((lesson, i) => (
                <tr key={lesson.id} style={{ backgroundColor: i % 2 === 0 ? "#ffffff" : "#f5f5f5" }}>
                  ...
                  <td className="px-4 py-3 border border-gray-200 text-center">
                    ...
                    >
                      {lang === "ar" ? "œ—”" : lang === "nl" ? "Les" : lang === "fr" ? "LeÁon" : "Lesson"}
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <p className="text-xs text-gray-400 mt-3">
          {lang === "ar" ? `≈Ã„«·Ì: ${filteredLessons.length} œ—”` : lang === "nl" ? `Totaal: ${filteredLessons.length} lessen` : lang === "fr" ? `Total: ${filteredLessons.length} leÁons` : `Total: ${filteredLessons.length} lessons`}
        </p>
      </div>
    </div>
  );
